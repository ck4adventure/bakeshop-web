...
// TODO: expand to 'low' | 'critical' once par / next-day quota data
// is available from the production schedule module
type Status = 'good' | 'zero';

function getStatus(quantity: number): Status {
  return quantity === 0 ? 'zero' : 'good';
}

const STATUS_ORDER: Record<Status, number> = { zero: 0, good: 1 };
...
const STATUS_CONFIG: Record<Status, {
  ...
  good: {
    label: 'In Stock',
    barColor: 'var(--status-above-par-text)',
    pillBg: 'var(--status-above-par-bg)',
    pillText: 'var(--status-above-par-text)',
  },
  zero: {
    label: 'Empty',
    barColor: 'var(--status-zero-text)',
    pillBg: 'var(--status-zero-bg)',
    pillText: 'var(--status-zero-text)',
  },
};
...
function StatusBar({ status }: { status: Status }) {
  // TODO: replace with real (quantity / par * 100) once par is available
  const pct = status === 'zero' ? 0 : 50;
  const cfg = STATUS_CONFIG[status];
  return (
...
export default function InventoryPage() {
  ...
  const emptyCount = inventory.filter(i => getStatus(i.quantity) === 'zero').length;

  const displayed = [...inventory]
    .filter(i => filter === 'all' || getStatus(i.quantity) === filter)
    .sort((a, b) => STATUS_ORDER[getStatus(a.quantity)] - STATUS_ORDER[getStatus(b.quantity)]);
    ...
    <div className="min-h-screen bg-background">
      ...
      <header className="sticky top-0 z-10 bg-card border-b border-border px-4 pt-5 pb-3">
        ...
        <div className="flex gap-2 mt-3">
          {([['all', 'All'], ['zero', 'Empty']] as const).map(([val, label]) => (
            <button
              key={val}
      ...
      <main className="px-4 pt-3 pb-24 flex flex-col gap-2">
        ...
        {!loadingInventory && !fetchError && displayed.length === 0 && (
